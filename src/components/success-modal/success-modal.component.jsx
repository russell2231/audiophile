...
const SuccessModal = () => {
	const { prevCart } = useContext(CartContext);

	const firstItem = prevCart[0];
	const totalQuantity = prevCart.reduce((acc, item) => acc + item.quantity, 0);

	const totalPrice =
		prevCart.reduce((acc, item) => acc + item.price * item.quantity, 0) || 0;
